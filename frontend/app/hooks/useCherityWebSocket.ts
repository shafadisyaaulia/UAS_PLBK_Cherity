...
/**
 * CHERITY WebSocket Hook
 * Custom React hook for connecting to CHERITY backend via WebSocket
 */
...
interface UseCherityWebSocketOptions {
  url?: string
  autoConnect?: boolean
...
export function useCherityWebSocket(options: UseCherityWebSocketOptions = {}) {
  const {
  ...
  const connect = useCallback(() => {
    try {
      ...
      ws.onopen = () => {
        console.log('✅ Connected to CHERITY backend')
        setIsConnected(true)
        setError(null)
      }
      ...
      ws.onclose = () => {
        console.log('🔌 Disconnected from CHERITY backend')
